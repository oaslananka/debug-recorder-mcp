...
function getFuzzyThreshold() {
    const configured = Number.parseFloat(process.env.FUZZY_THRESHOLD ?? `${DEFAULT_FUZZY_THRESHOLD}`);
...
function fuseOnlySearch(params, store) {
    const threshold = getFuzzyThreshold();
...
function queryFtsCandidates(query, limit, db, filters) {
...
export function searchSessions(params, store, db) {
    ...
    try {
        ...
        return rerankCandidates(params.query, candidates, params.limit).map((result) => ({
            ...result.item,
            _score: result.score
        }));
    }
...
function fuseOnlySimilar(errorMessage, store, limit) {
    const threshold = getFuzzyThreshold();
    const sessions = store.listSessions({ limit: 500, offset: 0 });
    const withErrors = sessions.filter((session) => Boolean(session.error_message));
...
export function findSimilarErrors(errorMessage, store, db, limit = 5) {
    const threshold = getFuzzyThreshold();
...
function addRelatedGroup(groups, reason, value, sessionId) {
    if (!value) {
        return;
    }
...
export function buildRelatedSessionGroups(results) {
    ...
    return [...groups.values()]
        ...
        return `${left.reason}:${left.value}`.localeCompare(`${right.reason}:${right.value}`);
    })
        .slice(0, 10);
}
function formatResultLine(result, index) {
    ...
    const score = result._score === undefined ? '' : ` score=${result._score.toFixed(3)}`;
    return `${index + 1}. **${result.title}** (${result.id}) — ${details.join(' / ') || 'no metadata'}${score}`;
}
export function formatSearchMarkdown(params, results, relatedGroups, pagination) {
    const lines = [
        '# Debug Search Export',
        '',
        `- Query: ${params.query}`,
    ...
    if (results.length === 0) {
        ...
        lines.push(...results.map((result, index) => formatResultLine(result, index)));
    }
    if (relatedGroups.length > 0) {
        lines.push('', '## Related session groups', '');
        for (const group of relatedGroups) {
            lines.push(`- ${group.reason}: ${group.value} — ${group.count} sessions (${group.session_ids.join(', ')})`);
        }
    }
...
export function searchSessionsPage(params, store, db) {
    ...
    return {
        ...
        ...(params.markdown
            ? {
                markdown: formatSearchMarkdown(params, results, relatedGroups, pagination)
            }
            : {})
    };
}
