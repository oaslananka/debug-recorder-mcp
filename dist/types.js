...
export const SearchSchema = z.object({
    ...
    markdown: z
        .boolean()
        .default(false)
        .describe('Include a Markdown incident/postmortem export for the current page')
});
...
export const ImportSessionsSchema = z.object({
    payload: ExportPayloadSchema.describe('Export JSON previously produced by export_sessions'),
    skip_existing: z
        .boolean()
        .default(true)
        .describe('Skip records whose IDs already exist instead of failing the import')
});
...
export const StatsOutputSchema = z.object({
    ...
    byLanguage: z.array(z.object({ language: z.string(), count: z.number().int().min(0) })),
    topErrorTypes: z.array(z.object({ error_type: z.string(), count: z.number().int().min(0) })),
