...
const KEY_VALUE_PATTERN = /((?:token|secret|password|api[_-]?key)[^=\n:]{0,32}[=:]\s*)([^\s,\]}]+)/gi;
const AUTHORIZATION_HEADER_PATTERN = /(authorization[^=\n:]{0,32}[=:]\s*)(bearer\s+)?([^\s,\]}]+)/gi;
...
function redactString(value) {
    return value
        .replace(AUTHORIZATION_HEADER_PATTERN, (_match, prefix, scheme) => {
