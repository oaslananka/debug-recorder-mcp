[**debug-recorder-mcp**](../README.md)

***

[debug-recorder-mcp](../README.md) / mcp

# mcp

## Type Aliases

- [DebugRecorderRuntime](type-aliases/DebugRecorderRuntime.md)

## Functions

- [closeRuntime](functions/closeRuntime.md)
- [createDebugRecorderServer](functions/createDebugRecorderServer.md)
- [createRuntime](functions/createRuntime.md)
- [createToolHandlers](functions/createToolHandlers.md)
- [isClosedDatabaseError](functions/isClosedDatabaseError.md)
- [safeHandler](functions/safeHandler.md)
- [startStdioServer](functions/startStdioServer.md)
